...
say "\nNot koha-plugin-edifact-rainbow-books, exiting" && exit 0 unless $repo eq "koha-plugin-edifact-rainbow-books";
...
foreach my $repo (@repos) {
    next if $repo eq 'koha-plugin-edifact-rainbow-books';
    next if $repo eq 'koha-plugin-edifact-rainbow-books-docs';
